<%-- 
    Document   : cadastrarAgendamento
    Created on : 11/09/2018, 00:11:48
    Author     : pein2
--%>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="well" style="background: #FFFFFF; border-radius: 0px;">
                        ...
                        <hr>
                        <%
                            String cpf_aluno = "" + request.getParameter("cpf_aluno");

                            if (!cpf_aluno.equals("null")) {
                        %>
                        ...
                        <form action="submitAgendamento" method="POST">
                            <div class="form-group">
                                <label>CPF:</label>
                                <input type="text" id="cpf_aluno" name="cpf_aluno" class="form-control" readonly="readonly" value="<%=p.getCpf()%>">
                            </div>
                            <div class="form-group">
                                <label>SITUAÇÃO:</label>
                                <input type="text" id="situacao" name="situacao" class="form-control" readonly="readonly" value="Aguardando">
                            </div>
                            <div class="form-group">
                                <label>DATA AULA</label>
                                <input type="datetime-local" class="form-control" name="data" placeholder="2017/09/08 13:38:00">
                            </div>
                            <div class="form-group">
                                <label>INSTRUTOR</label>
                                <select id="cpf_intrutor" name="cpf_intrutor" class="form-control">
                                    <%
                                        pessoaDAO pessoa = new pessoaDAO();
                                        List<Pessoa> listInstrutor = pessoa.dadosInstrutor();

                                        for (Pessoa instrutor : listInstrutor) {
                                    %>
                                    <option value="<%=instrutor.getCpf()%>">Instrutor <%=instrutor.getNome()%></option>
                                    <% }%>                    
                                </select></br>
                            </div>
                            <button type="submit" name="submit" class="btn btn-success" value="Cadastrar">Enviar para Aprovação</button>
                        </form>                     
                    </div>
                </div>
            </div>
